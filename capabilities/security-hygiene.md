...
status: required
intentions:
- Tracked environment configuration does not contain plaintext API tokens or personal
  access tokens.
...
- rule_id: VAL-security-hygiene-01
  ...
  method: Scan tracked non-contract files for token prefixes and patterns such as
    github_pat_, ghp_, and sk-ant-; exclude contract documentation paths.
  pass_condition: No matches are found outside excluded contract docs (for example
    capabilities/, profiles/, os/, validation/ markdown docs).
  severity: blocker
- rule_id: VAL-security-hygiene-02
  assertion: Tracked shell startup files do not assign literal secret values.
  method: Inspect tracked shell init files for token-bearing variable assignments
...
- rule_id: VAL-security-hygiene-03
  ...
  method: Confirm documented policy exists for avoiding sensitive command leakage
    in shell history.
